...
We evaluate the program, sample the resulting distribution, and score the samples. The model's answer is therefore a full distribution rather than an interval; the p5/p50/p95 triple is how we summarise it, not how we score it.
...
Pinning ground truth is the hardest part of building this benchmark, and it's where a reader should be most sceptical. Three kinds appear in the bank, with different reliability. Lets see some examples:
...
We inherited this convention rather than deriving it. We haven't tested normal-in-log-space against alternatives, or 90% bands against 80% or 95%. It's standard and defensible; it wasn't chosen over competitors in an experiment.
...
G is the reference described above, in log space; F is the model's distribution, likewise in log space.
...
The split maps onto the ordinary train/dev/test division, and onto ARC-AGI's public/semi-private/private structure. "Sealed" means sealed from the public, not from the model. Grading a private-tier question means sending it to the model being evaluated, and every model on the board is called through OpenRouter, which routes to whichever upstream provider serves that model. OpenRouter itself does not train on inputs or outputs, but the providers behind it don't all make the same commitment, and we run OpenRouter's default routing rather than restricting to providers with a no-training or zero-retention policy. So a private question is invisible to anyone browsing the site, but not invisible to whichever provider answers it — the same exposure any API call carries, no more and no less.


## Results

The bank holds 109 questions — 64 with point ground truth, 45 with authored distributional ground truth — evaluated on 40 models. Of 4,360 model–question cells, 4,297 produced a scorable Squiggle program. The 63 exclusions (1.4%) are dropped rather than penalised; most are evaluation failures, the rest truncated or filtered responses. One model, claude-fable-5, produced a program on only 58% of the bank and is excluded from the figures below.

### Calibration


Models are substantially overconfident. Across all answered cells, the fraction of ground-truth mass falling inside a model's stated 90% interval is 0.552 (95% bootstrap CI [0.511, 0.591], resampling questions rather than cells, 2,000 resamples). Under the stricter reading — does the reference median land inside the interval at all — coverage is 0.570 ([0.524, 0.612]) against a nominal of 0.941 for a bank of this composition.

No model reaches nominal. Coverage ranges from 0.284 for llama-4-scout to 0.798 for gpt-5.6-luna, and even the best model on the board is more than ten points short of the confidence it declares. This is not a property of the weak end of the field: the result holds for every frontier model we evaluated.

The magnitude is easier to read in widths. The median stated interval spans 0.467 dex — a factor of 2.9 between p5 and p95, or σ ≈ 0.33 nats. If the entire shortfall were insufficient spread around correctly placed centres, intervals would need to be roughly 2.2× wider to reach nominal: a median span of about a decade rather than a third of one. That calculation assumes unbiased log-errors, which we have not verified. Separating displaced centres from insufficient spread requires the per-cell bias diagnostic, and we report it as future work.

Coverage varies sharply across questions, and the tail is diagnostic rather than merely hard. One point-truth question was missed by 38 of 40 models. When a field this wide misses a fixed number almost unanimously, the likelier explanation is an authoring error than a difficult estimate, and the item is under audit. Question-level coverage is the cheapest audit signal we have, and it is now part of the review pass on every edition.

Two cautions on reading these numbers. Coverage is not the metric the board ranks on: it is unbounded in width and so rewards precisely the hedging Cramér-log is built to penalise. We report it because it is the standard diagnostic for the claim at issue and because it is directly comparable to prior work. And the exclusions are not missing at random — a model that fails to emit a valid program on the questions it finds hardest has its coverage measured on an easier subset, which biases these figures upward by an unknown amount.
